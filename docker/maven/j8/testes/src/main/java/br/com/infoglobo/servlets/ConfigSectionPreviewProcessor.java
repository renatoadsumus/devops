...
public class ConfigSectionPreviewProcessor extends GenericProcessor {

	private static final String SECTION_DEFAULT = "config";

	private static final Logger LOG = Logger.getLogger(ConfigSectionPreviewProcessor.class);

	private static final String PREVIEW_URL = "%s?cspid=%s&csid=%s&com.escenic.stale=false&token=%s%s";
	...
	public ConfigSectionPreviewProcessor() {

		addSectionPreview(SECTION_DEFAULT, "Config Section");
	}
	...
	private Section getContentSection(Section configSection) {
		
		Section result = null;
		try {
			result = loader.getSectionByUniqueName(configSection.getOwnerPublicationId(), getContentSectionUniqueName(configSection));
		} catch (Exception e) {}

		if (result==null) {
			// fallback to front page
			try {
				result = loader.getSectionByUniqueName(configSection.getOwnerPublicationId(), "ece_frontpage");
			} catch (Exception e) {}
		}
		
		if (LOG.isDebugEnabled()) {
			LOG.debug("for config section " + getSectionInfo(configSection) + ", preview content section is: " + getSectionInfo(result));
		}
	...
	private String getContentSectionUniqueName(Section section) {
		...
		return section.getUniqueName().substring(section.getUniqueName().lastIndexOf('.') + 1);
	}
